...
def export_liked_songs():
    ...
    for offset in range(0, 1500, 50): # Loop through pages of saved tracks, starting at 0 and incrementing by 50 (max limit)
        liked_songs = sp.current_user_saved_tracks(limit=50, offset=offset) # Get the user's saved tracks for the current page
    ...
    df.to_csv("liked_songs.csv", index=False) # Save the DataFrame as a CSV file named "liked_songs.csv" without the index
